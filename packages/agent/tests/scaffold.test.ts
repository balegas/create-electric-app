...
describe("scaffold", () => {
	...
	it("clones KPB and overlays template files", async () => {
		...
		assert.ok(fs.existsSync(path.join(TEST_DIR, "src/routes/__root.tsx")), "__root.tsx exists")
	})

	it("overlays Electric template files", () => {
		assert.ok(fs.existsSync(path.join(TEST_DIR, "docker-compose.yml")), "docker-compose.yml exists")
		assert.ok(fs.existsSync(path.join(TEST_DIR, "Caddyfile")), "Caddyfile exists")
		assert.ok(fs.existsSync(path.join(TEST_DIR, "drizzle.config.ts")), "drizzle.config.ts exists")
		assert.ok(fs.existsSync(path.join(TEST_DIR, "postgres.conf")), "postgres.conf exists")
		assert.ok(fs.existsSync(path.join(TEST_DIR, ".env.example")), ".env.example exists")
		assert.ok(fs.existsSync(path.join(TEST_DIR, "src/db/schema.ts")), "db/schema.ts exists")
		assert.ok(
			fs.existsSync(path.join(TEST_DIR, "src/db/zod-schemas.ts")),
			"db/zod-schemas.ts exists",
		)
		assert.ok(fs.existsSync(path.join(TEST_DIR, "src/db/index.ts")), "db/index.ts exists")
		assert.ok(fs.existsSync(path.join(TEST_DIR, "src/db/utils.ts")), "db/utils.ts exists")
		assert.ok(
	...
	it("merges Electric dependencies into package.json", () => {
		...
		assert.ok(pkg.dependencies["@tanstack/electric-db-collection"], "electric-db-collection added")
	...
	it("removes stale pnpm-lock.yaml", () => {
		assert.ok(!fs.existsSync(path.join(TEST_DIR, "pnpm-lock.yaml")), "pnpm-lock.yaml removed")
	})
	...
	it("creates _agent/ working memory directory", () => {
		assert.ok(fs.existsSync(path.join(TEST_DIR, "_agent/errors.md")), "errors.md exists")
		assert.ok(fs.existsSync(path.join(TEST_DIR, "_agent/session.md")), "session.md exists")
	})

	it("does NOT inject ssr: false into root route (root must always SSR)", () => {
		const rootRoute = fs.readFileSync(path.join(TEST_DIR, "src/routes/__root.tsx"), "utf-8")
