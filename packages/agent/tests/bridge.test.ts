...
describe("stdio adapter roundtrip", () => {
	it("should read config, receive events, and handle gate responses via stdin/stdout", async () => {
		...
		const script = `
import { createStdioAdapter } from "${path.resolve(import.meta.dirname, "../src/engine/stdio-adapter.js")}"
...
`
		...
		try {
			...
			const logEvents = collectedEvents.filter((e) => e.type === "log" && "message" in e) as Array<{
				type: "log"
				message: string
			}>
			assert.ok(
